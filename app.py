...
# Page config
st.set_page_config(page_title="Earth Engine Analytics", layout="wide")

# Your client secret JSON content
CLIENT_SECRET_JSON = {
    "web": {
...
# For local testing, you can uncomment and use this:
# CLIENT_SECRET_JSON["web"]["redirect_uris"] = ["http://localhost:8501"]
...
def show_main_app():
    """Your main Earth Engine app content"""
    st.title("🌍 Earth Engine Analytics Dashboard")
    ...
    with st.sidebar:
        ...
        # Region selection
        region_options = ["India", "Europe", "North America", "South America", "Africa", "Asia"]
        selected_region = st.selectbox("Select Region", region_options, index=0)
        
        # Dataset selection
        dataset_options = ["Landsat 8", "Sentinel-2", "MODIS", "Weather Data"]
        selected_dataset = st.selectbox("Select Dataset", dataset_options, index=0)
        
        if st.button("🔄 Process Data", type="primary", use_container_width=True):
            st.session_state.processing = True
    
    # Main content area
    tab1, tab2, tab3, tab4 = st.tabs(["📊 Dashboard", "🗺️ Map", "📈 Analytics", "📁 Data"])
    ...
    with tab1:
        ...
        with col2:
            st.metric("Images Processed", "1,245", "+12%")
        with col3:
            st.metric("NDVI Change", "+0.15", "+5%")
        with col4:
            st.metric("Processing Time", "2.4s", "-15%")
        
        # Time series chart
        st.subheader("NDVI Trend Over Time")
        dates = pd.date_range(start=start_date, end=end_date, periods=30)
        values = [0.3 + 0.5 * (i/30) + 0.1 * (i % 5) for i in range(30)]
        
        fig = px.line(x=dates, y=values, title='NDVI Trend',
    ...
    with tab2:
        ...
        # Create a map based on selected region
        region_coords = {
        ...
        m = folium.Map(location=region_coords.get(selected_region, [20.5937, 78.9629]), zoom_start=5)
        
        # Add sample markers
        sample_locations = {
            "India": [{"name": "Delhi", "coords": [28.6139, 77.2090]},
                     {"name": "Mumbai", "coords": [19.0760, 72.8777]}],
            "Europe": [{"name": "Paris", "coords": [48.8566, 2.3522]},
                      {"name": "Berlin", "coords": [52.5200, 13.4050]}],
        }
        
        locations = sample_locations.get(selected_region, [{"name": "Center", "coords": region_coords.get(selected_region, [0, 0])}])
    ...
    with tab3:
        ...
        data = {
            'Category': ['Forest', 'Agriculture', 'Urban', 'Water', 'Barren'],
            'Area (km²)': [12000, 8000, 3000, 1500, 500],
            'Change (%)': [2.5, 1.8, -0.5, 0.2, -1.2]
        }
        ...
        with col1:
            fig1 = px.bar(df, x='Category', y='Area (km²)', 
                         title='Land Cover Distribution',
                         color='Category')
            st.plotly_chart(fig1, use_container_width=True)
        
        with col2:
            fig2 = px.pie(df, values='Area (km²)', names='Category',
    ...
    with tab4:
        ...
        sample_data = pd.DataFrame({
            ...
            'NDVI': [0.45, 0.48, 0.52, 0.49, 0.51, 0.53, 0.55, 0.52, 0.54, 0.56],
            'Temperature': [28.5, 29.1, 30.2, 29.8, 30.5, 31.0, 30.8, 30.2, 29.9, 30.5],
            'Rainfall': [12.5, 0, 8.2, 15.3, 3.5, 0, 5.2, 10.8, 2.1, 0]
        })
        ...
        with col1:
            if st.button("📥 Download CSV", use_container_width=True):
                ...
                st.download_button(
                    label="Download CSV",
                    data=csv,
                    file_name="earth_engine_data.csv",
                    mime="text/csv"
                )
        with col2:
...
if st.session_state.credentials:
    try:
        # Build service and get user info
    ...
    if code:
        ...
        # Show login screen
        st.markdown('<div class="main-header"><h1>🌍 Earth Engine Analytics Platform</h1></div>', unsafe_allow_html=True)
        ...
        with col2:
            ...
            try:
                ...
                st.info(f"**Redirect URI:** {CLIENT_SECRET_JSON['web']['redirect_uris'][0]}")
        ...
        <div style="text-align: center; color: #666; padding: 20px;">
            <p>🌱 Earth Engine Analytics Platform • Powered by Google Earth Engine</p>
            <p>📧 Contact: support@earthengine.com</p>
        </div>
        """, unsafe_allow_html=True)
